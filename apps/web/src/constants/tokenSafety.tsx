    ...
    switch (warning.level) {
      ...
      case WARNING_LEVEL.UNKNOWN:
        heading = (
          <Plural
            value={plural ? 2 : 1}
            one={t(`{{name}} isn't traded on leading U.S. centralized exchanges or frequently swapped on Udonswap.`, {
              name: tokenSymbol ?? 'This token',
            })}
            other="These tokens aren't traded on leading U.S. centralized exchanges or frequently swapped on Udonswap."
          />
        )
        description = <Trans>Always conduct your own research before trading.</Trans>
        break
      case WARNING_LEVEL.BLOCKED:
        description = (
          <Plural
            value={plural ? 2 : 1}
            one={t(`You can't trade {{name}} using the Udonswap App.`, {
              name: tokenSymbol ?? 'this token',
            })}
            other="You can't trade these tokens using the Udonswap App."
          />
        )
        break
    }
