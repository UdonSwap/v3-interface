...
test.each(tokens)('should inject metadata for valid tokens', async (token) => {
  ...
  expect(body).toContain(`<meta property="og:title" content="Get ${token.symbol} on Udonswap"/>`)
  ...
  expect(body).toContain(`<meta property="og:image:alt" content="Get ${token.symbol} on Udonswap"/>`)
  expect(body).toContain(`<meta property="twitter:card" content="summary_large_image"/>`)
  expect(body).toContain(`<meta property="twitter:title" content="Get ${token.symbol} on Udonswap"/>`)
  expect(body).toContain(`<meta property="twitter:image" content="${token.image}"/>`)
  expect(body).toContain(`<meta property="twitter:image:alt" content="Get ${token.symbol} on Udonswap"/>`)
})
