...
test.each(defaultUrls)('should inject metadata for valid collections', async (defaultUrl) => {
  ...
  expect(body).toContain(`<meta property="og:title" content="Udonswap Interface"/>`)
  expect(body).toContain(
    `<meta property="og:description" content="Swap or provide liquidity on the Udonswap Protocol"/>`
  )
  expect(body).toContain(
  ...
  expect(body).toContain(`<meta property="og:image:alt" content="Udonswap Interface"/>`)
  expect(body).toContain(`<meta property="twitter:card" content="summary_large_image"/>`)
  expect(body).toContain(`<meta property="twitter:title" content="Udonswap Interface"/>`)
  expect(body).toContain(
    `<meta property="twitter:image" content="http://127.0.0.1:3000/images/1200x630_Rich_Link_Preview_Image.png"/>`
  )
  expect(body).toContain(`<meta property="twitter:image:alt" content="Udonswap Interface"/>`)
})
